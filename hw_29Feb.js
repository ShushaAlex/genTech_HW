...
db.employees.find({$and: [{salary: {$gte: 4500}}, {salary: {$lte: 5500}}]}).sort({age: 1});
...
db.employees.find({$and: [{age: {$gt: 25}}, {age: {$lt: 35}}]}).limit(3);
...
db.employees.find({position: "Analyst"}, {_id: 0, name: 1, position: 1}).sort({age: -1}).skip(1);

//Есть коллекция departments:
...
db.departments.aggregate([
  ...
  },
  {
    $project: {
      _id: 0,
      first_emploee: { $arrayElemAt: ["$employees", 0] },
      second_emploee: { $arrayElemAt: ["$employees", 1] }
    }
  }
])
...
db.departments.aggregate([
  ...
  },
  {
    $project: {
      _id: 0,
      emploee1: { $arrayElemAt: ["$employees", -1] },
      emploee2: { $arrayElemAt: ["$employees", -2] },
      emploee3: { $arrayElemAt: ["$employees", -3] }
    }
  }
]);
...
db.departments.aggregate([
  ...
  },
  {
    $project: {
      _id: 0,
      emploee3: { $arrayElemAt: ["$employees", 2] },
      emploee4: { $arrayElemAt: ["$employees", 3] }
    }
  }
]);
